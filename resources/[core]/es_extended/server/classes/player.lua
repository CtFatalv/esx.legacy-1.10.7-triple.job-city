...
function CreateExtendedPlayer(playerId, identifier, group, accounts, inventory, weight, job, job2, job3, loadout, name, coords, metadata)
    ...
    function self.setJob2(newJob2, grade2)
        ...
        if not ESX.DoesJob2Exist(newJob2, grade2) then
    ...
    ---@param newJob3 string
    ---@param grade3 string
    ---@return void
    function self.setJob3(newJob3, grade3)
        grade3 = tostring(grade3)
        local lastJob3 = self.job3

        if not ESX.DoesJob3Exist(newJob3, grade3) then
            return print(("[es_extended] [^3WARNING^7] Ignoring invalid ^5.setJob3()^7 usage for ID: ^5%s^7, Job3: ^5%s^7"):format(self.source, job3))
        end

        local job3Object, grade3Object = ESX.Jobs[newJob3], ESX.Jobs[newJob3].grades[grade3]
