...
function Core.SavePlayer(xPlayer, cb)
    ...
    MySQL.prepare(
        "UPDATE `users` SET `accounts` = ?, `job` = ?, `job_grade` = ?, `job2` = ?, `job2_grade` = ?, `job3` = ?, `job3_grade` = ?, `group` = ?, `position` = ?, `inventory` = ?, `loadout` = ?, `metadata` = ? WHERE `identifier` = ?",
        parameters,
...
function ESX.DoesJob2Exist(job2, grade2)
    return (ESX.Jobs[job2] and ESX.Jobs[job2].grades[tostring(grade2)] ~= nil) or false
end

function ESX.DoesJob3Exist(job3, grade3)
    return (ESX.Jobs[job3] and ESX.Jobs[job3].grades[tostring(grade3)] ~= nil) or false
end
