...
function OpenKeysMenu()
    ...
    ESX.TriggerServerCallback('Alf-Carkeys:getPlayersKeys', function(keys)
        ...
        ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'CarKeys', {
            title    = 'Système de clé - clé du véhicule',
            align    = 'top-left',
            elements = elements
        }, function(data, menu)
            menu.close()
            ...
            align    = 'top-left',
            elements = {{label = 'Donner', value = 'giveKey'}, {label = 'Renommer', value = 'renameKey'}, {label = 'Supprimer le nom', value = 'removeKeyName'}, {label = 'Supprimer', value = 'deleteKey'}}
            }, function(data2, menu2)
                menu2.close()
                    if data2.current.value == 'renameKey' then
                        ESX.UI.Menu.Open('dialog', GetCurrentResourceName(), 'menu1',{
                            title = ('Nouveau nom (Max ' ..Config.RenameKeyLenght.. ' caractères)')
                        },function(data3, menu)
                    ...
                    elseif data2.current.value == 'giveKey' then
                        ...
                        if data.current.id and data.current.state == "Original" then 
                            ESX.ShowNotification(_U('CantGiveOriginal'))
						elseif reciever ~= -1 and distance <= 3.0 then
                    ...
                    elseif data2.current.value == 'deleteKey' then
                        ESX.TriggerServerCallback('Alf-Carkeys:getPlayersKeys', function(keys)
                            for k,key in ipairs(keys) do
                                ...
                                elseif key.state == "Copie" or key.state == "Garagenzugriff" then
...
function OpenCopyKeysMenu()
    ...
    local playerId = PlayerId()
        ...
        ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'CarKeyMenu', {
            title    = 'Système de clé - serrurier',
            align    = 'top-left',
            elements = {
            ...
            if data.current.value == 'copykey' then
                ...
                ESX.TriggerServerCallback('Alf-Carkeys:getPlayersKeysOriginal', function(keys)
                    ...
                    ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'CarCopyKeys', {
                        ...
                        title    = 'Clé du véhicule - ' ..data.current.label,
                        align    = 'top-left',
                        elements = {{label = 'Copie - <span style="color: green;">$'..Config.KeyPrice..'</span>', value = 'copyKey'}}
                        }, function(data2, menu2)
                            menu2.close()
            ...
            elseif data.current.value == 'trustedkey' then
                ...
                ESX.TriggerServerCallback('Alf-Carkeys:getPlayersKeysOriginal', function(keys)
                    ...
                    ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'trustedkeymenu', {
                        ...
                        title    = 'Clé du véhicule - ' ..data.current.label,
                        align    = 'top-left',
                        elements = {{label = 'Copie - <span style="color: green;">$'..Config.TrustedKeyPrice..'</span>', value = 'trustedkey'}}
                        }, function(data2, menu2)
                            menu2.close()
...
function OpenTrustedCopyKeysMenu()
    ...
    local playerId = PlayerId()
        ...
        ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'CarKeyMenu', {
            ...
            elements = {
                {label = ('Copier la clé'), value = 'copykey'},
                {label = ('Clé d\'accès du garage'), value = 'trustedkey'}
            }
        }, function(data, menu)
            menu.close()
...
function OpenChangePlateMenu()
    ...
    ESX.TriggerServerCallback('Alf-Carkeys:getPlayersKeysOriginal', function(keys)
            ...
            align    = 'top-left',
            elements = {{label = 'Changer la plaque d\'immatriculation - <span style="color: green;">$'..Config.ChangePlatePrice..'</span>', value = 'ChangePlate'}}
            }, function(data2, menu2)
                menu2.close()
...
AddEventHandler('Alf-Carkeys:hasEnteredMarker', function (zone)
    if zone == 'CopyKeyStore' then
        ...
        CurrentActionMsg = ('Appuyer sur ~INPUT_CONTEXT~ pour ouvrir le menu')
...
function VehiclePassengerControls()
    ...
    ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'VehicleControls', {
        title    = 'Système de clé - véhicule',
        align    = 'top-left',
        elements = {
            {label = ('Ouvrir/fermer la porte'), value = 'door'},
...
function BoatControls()
    ...
    ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'BoatControls', {
        title    = 'Système de clé - Démarrage',
        align    = 'top-left',
        elements = {
            {label = ('Moteur'), value = 'engine'},
            {label = ('Ouvrir / Fermer'), value = 'doorlock'},
        ...
        elseif data.current.value == 'anchor' then
            ...
            if anchor == false then
                ...
                ESX.ShowNotification("~g~ancré")
                anchor = true
            elseif anchor == true then
                SetBoatAnchor(boat, false)
                ESX.ShowNotification("~r~n'est plus ancré")
                anchor = false
            end
        end
    end, function(data, menu)
